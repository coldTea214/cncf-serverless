...
serving 引入了 Service(ksvc)、Route(rt)、Configuration(cfg) 和 Revision(rev) 4个 CRD。4者的关系是：
...
除了上面提及的 ksvc、rt、cfg 和 rev 外，实际上 Knative 还有一些 CRD：Ingresses(king)，ServerlessService(sks)，PodAutoscaler(kpa)，这些会在使用小节结合实例说明
...
在 knative 体系下编写 go 应用，跟常规 go web 开发并无区别
...
$ docker build . -t dev.local/hello-go:test
```
